...
一个PQT的wiki机器人，基于qq平台运行，用于查询游戏数据，通常为角色、女神。
...
现在如果没有命中标准名称或别名时，会根据匹配命中的字符给出建议

2、优化了指令的用法

由原先的"/wiki [角色]"改为"[角色]"，即删除了/wiki前缀，但兼容原来的方式

### 202604

1、新增抓取最新数据指令

抓取一个，不存在于当前数据库中的词条（注，如果删除数据的话，不一定对得上）